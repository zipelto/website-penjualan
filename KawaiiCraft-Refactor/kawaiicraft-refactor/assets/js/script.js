...
const HARGA = {
  ...
  flanel: 12000,   // Bingkai Foto Flanel 2-in-1 — HPP Rp3.500
...
const products = [
  {id:9, type:"flanel", name:"Bingkai Flanel Polos", category:"flanel", layout:"flanel", badge:"Unggulan",
   theme:"pink", price:HARGA.flanel, maxPhotos:0,
  ...
  {id:10, type:"flanel", name:"Bingkai Flanel Telinga Kucing", category:"flanel", layout:"flanel",
   theme:"biru", price:HARGA.flanel, maxPhotos:0,
   desc:"Versi dengan hiasan timbul telinga kucing dan manik-manik di bagian depan."},

  {id:7, type:"poster", name:"Poster Anime Ready Stock", category:"anime", layout:"band", badge:"Populer",
  ...
  {id:12, type:"paket", name:"Paket Kombinasi", category:"paket", layout:"flanel", badge:"Hemat",
...
const SLOTS = {polaroid:1, band:1, grid2:2, strip3:3, collage4:4, flanel:1};
...
function cardHTML(p){
  return `<article class="pcard">
    ...
    <div class="body">
      ...
      <button class="btn sm" onclick="openDetail(${p.id})">Atur &amp; pesan</button>
    </div>
  </article>`;
}
...
function openDetail(id){
  ...
  box.innerHTML = `
    <div><div class="poster" id="detailPoster"></div></div>
    <div>
      <div style="color:var(--ink-soft);font-size:.85rem">${catLabel(p.category)}</div>
      <h2 style="margin-top:.2rem">${p.name}</h2>
      <p style="color:var(--ink-soft)">${p.desc}</p>
      <div class="price-big">${rupiah(p.price)}</div>
      <div style="font-size:.85rem;color:var(--ink-soft);margin-bottom:1rem">per ${p.type === "paket" ? "paket" : "pcs"}, belum termasuk ongkir</div>
      <ul class="spec">${specList(p)}</ul>
      <button class="btn" onclick="go('custom')" style="width:100%">Atur ${p.type === "poster" ? "poster" : "pesanan"} ini</button>
    </div>`;
  paint(document.getElementById("detailPoster"), previewCfg(p));
  go("detail");
}
...
function initCustom(){
  ...
  show("wrapPhoto", isPoster && p.maxPhotos > 0);
  ...
  if(isPoster && p.maxPhotos > 0){
    document.getElementById("photoHelp").textContent =
      `— ${SLOTS[p.layout] || 1} slot di template ini, maksimal ${p.maxPhotos} foto`;
  }
  drawThumbs();
  refresh();
}
...
function refresh(){
  ...
  paint(document.getElementById("preview"), previewCfg(p, {
    theme:state.theme, font:state.font, name:state.name, title:state.title,
    quote:state.quote || (p.type !== "poster" && state.hiasan !== "Tanpa hiasan" ? "hiasan " + state.hiasan.toLowerCase() : ""),
    photos:state.photos
  }));
  ...
  w.innerHTML = (p.type !== "flanel" && p.maxPhotos > 0 && state.photos.length === 0)
    ? `<div class="warn">Belum ada foto yang diunggah. Kamu tetap bisa memesan — fotonya nanti dikirim lewat chat WhatsApp.</div>`
    : (p.type === "flanel"
        ? `<div class="warn">Bingkai dikirim kosong dengan kantong foto siap pakai, jadi kamu bisa memasukkan photocard atau foto sendiri kapan saja.</div>` : "");
}
...
fileInput.onchange = e => {
  const max = state.product.maxPhotos;
  const files = [...e.target.files].slice(0, max - state.photos.length);
  files.forEach(f=>{
    const r = new FileReader();
    r.onload = ev => { state.photos.push(ev.target.result); drawThumbs(); refresh(); };
    r.readAsDataURL(f);
  });
  fileInput.value = "";
};
function drawThumbs(){
  document.getElementById("thumbs").innerHTML = state.photos.map((src,i)=>
    `<figure><img src="${src}" alt="Foto ${i+1}"><button onclick="delPhoto(${i})" aria-label="Hapus foto ${i+1}">×</button></figure>`).join("");
}
function delPhoto(i){ state.photos.splice(i,1); drawThumbs(); refresh(); }
...
function itemDetail(it){
  ...
  if(it.type !== "flanel") bits.push(it.photos ? it.photos + " foto diunggah" : "foto menyusul lewat chat");
...
/* ============================================================
   NAVIGASI
   ============================================================ */
const VIEWS = ["home","katalog","detail","custom","cara","tentang"];
function go(v){
...
document.getElementById("backDetail").onclick = () => state.product ? openDetail(state.product.id) : go("katalog");
